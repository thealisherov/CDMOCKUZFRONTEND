...
// ODDIY PLATFORMA testlari (center_id YO'Q). Volume 6 bilan bir xil konventsiya:
// type='reading', test_id="Reading Volume 8.N", data.id="Volume Reading 8_N".
const TESTS = [1, 2, 3, 4, 5, 6, 7, 8]
  .map((n) => ({ n, jsonPath: join(__dirname, '..', 'testlar', `volume8-reading-test-${n}.json`) }))
  .filter(({ jsonPath }) => existsSync(jsonPath))
  .map(({ n, jsonPath }) => ({
    testId: `Reading Volume 8.${n}`,
