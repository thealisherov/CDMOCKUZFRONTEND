...
// Reading JSON ni fayldan o'qiymiz
const jsonPath = join(__dirname, '..', 'testlar', 'masterielts_reading_v8_t5.json');
const testData = JSON.parse(readFileSync(jsonPath, 'utf-8'));

const TEST_ID = testData.id;          // "masterielts-reading-v8-t5"
...
async function run() {
  ...
  const { data: row } = await supabase
    .from('Tests').select('id, center_id').eq('test_id', TEST_ID).maybeSingle();
  if (row?.center_id) {
    console.log(`🎯 center_id avtomatik ulandi: ${row.center_id} (Test row ID: ${row.id})`);
  } else {
    console.log(`⚠️  center_id ulanmadi.`);
  }
}
