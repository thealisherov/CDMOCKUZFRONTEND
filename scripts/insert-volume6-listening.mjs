...
// Bu testlar ODDIY PLATFORMA testlari (center_id YO'Q — barchaga ochiq),
// Volume 6 reading bilan bir xil konventsiya: type='listening',
// test_id="Listening Volume 6.N", data.id="Volume Listening 6_N".
const TESTS = [1, 2, 3, 4, 5, 6]
  .map((n) => ({ n, jsonPath: join(__dirname, '..', 'testlar', `volume6-listening-test-${n}.json`) }))
  .filter(({ jsonPath }) => existsSync(jsonPath))
  .map(({ n, jsonPath }) => ({
    testId: `Listening Volume 6.${n}`,
