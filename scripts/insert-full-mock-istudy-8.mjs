...
const jsonPath = join(__dirname, '..', 'testlar', 'full_mock_istudy_8.json');
const testData = JSON.parse(readFileSync(jsonPath, 'utf-8'));

const TEST_ID = testData.id;          // "istudy-full-mock-8"
...
async function run() {
  ...
  if (row?.center_id) {
    ...
    console.log(`⚠️  center_id NULL qoldi. Trigger ishlamagan bo'lishi mumkin.`);
    // Qo'lda bog'laymiz agar center topilgan bo'lsa
    if (center?.id) {
      const { error: patchErr } = await supabase
        .from('Tests').update({ center_id: center.id }).eq('test_id', TEST_ID);
      if (patchErr) console.log(`   Qo'lda bog'lashda xato: ${patchErr.message}`);
      else console.log(`   Qo'lda bog'landi: center_id = ${center.id}`);
    }
  }
}
