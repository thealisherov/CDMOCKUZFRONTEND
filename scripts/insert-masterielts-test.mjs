...
// Listening JSON ni fayldan o'qiymiz
const jsonPath = join(__dirname, '..', 'testlar', 'masterielts_listening_v6_t4.json');
const testData = JSON.parse(readFileSync(jsonPath, 'utf-8'));

const TEST_ID = testData.id;          // "masterielts-listening-v6-t4"
const CENTER_SLUG = testData.center;  // "masterielts"
...
async function run() {
  ...
  if (!center) {
    ...
    console.log(`   Avval Admin Panel → Centers dan "${CENTER_SLUG}" markazini yarating.`);
  } else {
  ...
  if (existing) {
    ({ error: err } = await supabase.from('Tests')
      .update({ data: testData, type: TYPE }).eq('test_id', TEST_ID));
    console.log(err ? `❌ ${err.message}` : `✅ Yangilandi: ${testData.title}`);
  } else {
    ({ error: err } = await supabase.from('Tests')
      .insert({ test_id: TEST_ID, type: TYPE, data: testData }));
  ...
  if (row?.center_id) {
    console.log(`🎯 center_id avtomatik ulandi: ${row.center_id}`);
  } else {
    console.log(`⚠️  center_id ulanmadi (agar markaz hali yaratilmagan bo'lsa, markaz ochilgach avtomatik sinxronlanadi).`);
  }
}
