...
export default function LeaderboardClient({ initialData }) {
  ...
  const { data, error, isLoading } = useQuery({
    ...
    staleTime: 60 * 1000,
    // Use server-prefetched data as initial data
    initialData: initialData?.leaderboard?.length > 0 ? initialData : undefined,
  });
  ...
  if (isLoading && !initialData?.leaderboard?.length) return (
